...
void ElementwiseCompute<T, Functor, PType>::Run() {
  ...
  if (param.enable_int8) {
    ...
    for (size_t i = 0; i < x_dim.size(); i++) {
      ...
      // In current case,only need support braoadcast y.
      if (y_shape[i] == 1 && x_shape[i] > 1) {
        broadcast_y = true;
      }

      if (x_shape[i] == 1) {
        CHECK_EQ(x_shape[i], y_shape[i]);
      }
    }

    const T* real_y = nullptr;
    if (broadcast_y) {
      VLOG(1) << "broadcast1:";
      broadcast_y_guard_->Reserve(len);
      int r = xdnn::broadcast<int8_t>(
          ctx.GetRawContext(),
          y->template data<int8_t>(),
          reinterpret_cast<int8_t*>(broadcast_y_guard_->addr_),
          y_shape,
          x_shape);
      VLOG(1) << "broadcast2:";
      real_y = reinterpret_cast<T*>(broadcast_y_guard_->addr_);
      CHECK_EQ(r, 0);
    } else {
      real_y = y->template data<T>();
    }
  ...
  if (!param.enable_int8) {
    if (x_dim.size() == y_dim.size()) {
      for (size_t i = 0; i < x_dim.size(); i++) {
        ...
        y_shape[i] = static_cast<int>(y_dim[i]);
        CHECK_EQ(
            (x_shape[i] == y_shape[i] || x_shape[i] == 1 || y_shape[i] == 1),
            true);
      }
...
REGISTER_LITE_KERNEL(
    elementwise_add, kXPU, kInt8, kNCHW, AddInt8, DISABLE_XPU1_AddInt8)
...
REGISTER_LITE_KERNEL(
    elementwise_mul, kXPU, kInt8, kNCHW, MulInt8, DISABLE_XPU1_MulInt8)
    .BindInput("X", {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kInt8))})
    .BindInput("Y", {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kInt8))})
    .BindOutput("Out", {LiteType::GetTensorTy(TARGET(kXPU), PRECISION(kInt8))})
    .Finalize();
